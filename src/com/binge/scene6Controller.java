...
public class scene6Controller implements Initializable {
    ...
    horse horse;
    public static time time = scene5Controller.time;

    @Override
    public void initialize(URL location, ResourceBundle resources) {
        // 確保 gridpane 已經載入完成
        horse = new horse(10, 0, 10, gridpane6);
    ...
    public void onKnightMoved() {
        if (horse.row == 0 && horse.col == 10) {
            switchToScene7(); // 到達目標位置就轉場
        }
    }

    public void switchToScene7() {
        try {
            Parent root = FXMLLoader.load(getClass().getResource("scene7.fxml"));
            Stage stage = (Stage) gridpane6.getScene().getWindow();
